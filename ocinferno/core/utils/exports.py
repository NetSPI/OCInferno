"""Data-export + compartment-tree SVG rendering.

Split out of module_helpers.py: the CSV/JSON/Excel blob exporters and the
compartment-tree SVG renderer. Kept import-light (pandas/xlsxwriter are imported
lazily inside the Excel path so the optional [excel] extra stays optional).
module_helpers re-exports the public functions for backwards compatibility.
"""
    ...
    *,
    ...
    Behavior:
      ...
      - condensed=True: output `Table Name`, `Compartment ID`, `Compartment Name`,
        `Service Category`, `Resource Category`, `Resource Display Name`, `Remaining JSON`
    """
    ...
    try:
        ...
        raise RuntimeError(
            "Excel export requires the optional Excel dependencies. Install them with "
            "`pip install ocinferno[excel]` (or `pip install pandas xlsxwriter`)."
        ) from e
    ...
    def _build_condensed_record(db_info: Dict[str, Any], table_name: str, rd: Dict[str, Any]) -> Dict[str, Any]:
        # Lazy import: resource_type_area lives under modules/, exports.py under core/ --
        # avoid a module-load-time core->modules dependency, matching this function's
        # existing lazy-pandas-import pattern.
        from ocinferno.modules.everything.utilities.enum_all_summary import resource_type_area
...
def _row_simple_resource_display_name(row: Dict[str, Any]) -> tuple[str, str]:
    ...
    # Fallback: the curated list above only covers the most common columns --
    # many tables use a service-specific "<noun>_name" column instead (e.g.
    # vault_secret.secret_name, databases.db_unique_name, identity_domain_apps.
    # app_name), which would otherwise silently leave "Resource Display Name"
    # blank. Try any remaining column whose name ends in "name", picked in a
    # stable (sorted) order so the result is deterministic.
    for key in sorted(row.keys()):
        if key in name_keys:
            continue
        if not str(key).lower().endswith("name"):
            continue
        s = _resource_display_value(row.get(key))
        if s:
            return s, key
