#!/usr/bin/env python3
"""OCI Core Services: Compute, Compute Management, Virtual Network, Block Storage.

OCI bundles these under one "Core Services" umbrella, so (matching every other
service module in this repo) they live in a single ``utilities/helpers.py``
rather than separate per-concept files.

Most Resource classes here are plain ``OciListResource`` subclasses (see
``ocinferno.core.resource``) -- config-only, no hand-rolled list/get/save
bodies. The exception is ``ComputeInstanceAgentResourceClient`` (run-command /
execution / plugin machinery): three SDK clients, no single list/get pair, and
heavy per-row normalization make it a poor fit for the shared base, so it (and
its free-function helpers below it) keep their original hand-rolled shape.
"""
...
# =============================================================================
# Shared: forward an optional list-scope kwarg only when it's truthy
# =============================================================================

class _OptionalScopeKwargMixin:
    """Forward ``SCOPE_KWARG_NAME`` to the SDK list call only when truthy.

    Several list_* SDK methods across these clients accept an extra
    scope-narrowing kwarg (``vcn_id``, ``drg_id``, ``availability_domain``).
    The pre-migration wrappers only added the kwarg to the call when a value
    was actually supplied (``if x: kwargs["x"] = x``) rather than always
    passing it (even as ``None``) -- replicate that exactly, since an
    explicit ``None`` is not the same SDK call shape as omitting the kwarg.
    """

    SCOPE_KWARG_NAME: str = ""

    def _list_items(self, compartment_id: str, **kwargs):
        name = self.SCOPE_KWARG_NAME
        value = kwargs.pop(name, None) if name else None
        if value:
            kwargs[name] = value
        return super()._list_items(compartment_id, **kwargs)


# =============================================================================
# Block Storage (oci.core.BlockstorageClient)
# =============================================================================

class BlockVolumesResource(OciListResource):
    CLIENT_CLS = oci.core.BlockstorageClient
    SERVICE_NAME = "Block Storage"
    TABLE_NAME = "blockstorage_volumes"
    LIST_METHOD = "list_volumes"
    GET_METHOD = "get_volume"
    GET_ID_PARAM = "volume_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "size_in_gbs", "availability_domain"]


class BlockBootVolumesResource(_OptionalScopeKwargMixin, OciListResource):
    SCOPE_KWARG_NAME = "availability_domain"
    CLIENT_CLS = oci.core.BlockstorageClient
    SERVICE_NAME = "Block Storage"
    TABLE_NAME = "blockstorage_boot_volumes"
    LIST_METHOD = "list_boot_volumes"
    GET_METHOD = "get_boot_volume"
    GET_ID_PARAM = "boot_volume_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "size_in_gbs", "availability_domain"]


class BlockVolumeBackupsResource(OciListResource):
    CLIENT_CLS = oci.core.BlockstorageClient
    SERVICE_NAME = "Block Storage"
    TABLE_NAME = "blockstorage_volume_backups"
    LIST_METHOD = "list_volume_backups"
    GET_METHOD = "get_volume_backup"
    GET_ID_PARAM = "volume_backup_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "size_in_gbs", "volume_id"]


class BlockBootVolumeBackupsResource(OciListResource):
    CLIENT_CLS = oci.core.BlockstorageClient
    SERVICE_NAME = "Block Storage"
    TABLE_NAME = "blockstorage_boot_volume_backups"
    LIST_METHOD = "list_boot_volume_backups"
    GET_METHOD = "get_boot_volume_backup"
    GET_ID_PARAM = "boot_volume_backup_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "size_in_gbs", "boot_volume_id"]


# =============================================================================
# Virtual Network (oci.core.VirtualNetworkClient)
# =============================================================================
...
class SubnetsResource(_OptionalScopeKwargMixin, OciListResource):
    SCOPE_KWARG_NAME = "vcn_id"
    CLIENT_CLS = oci.core.VirtualNetworkClient
    SERVICE_NAME = "Virtual Network"
    TABLE_NAME = "virtual_network_subnets"
    LIST_METHOD = "list_subnets"
    GET_METHOD = "get_subnet"
    GET_ID_PARAM = "subnet_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "cidr_block", "vcn_id"]
    SECTION_TITLE = "Subnets"


class RouteTablesResource(_OptionalScopeKwargMixin, OciListResource):
    SCOPE_KWARG_NAME = "vcn_id"
    CLIENT_CLS = oci.core.VirtualNetworkClient
    SERVICE_NAME = "Virtual Network"
    TABLE_NAME = "virtual_network_route_tables"
    LIST_METHOD = "list_route_tables"
    GET_METHOD = "get_route_table"
    # SDK signature is get_route_table(self, rt_id, **kwargs) -- confirmed via
    # oci/core/virtual_network_client.py:get_route_table.
    GET_ID_PARAM = "rt_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "vcn_id"]
    SECTION_TITLE = "Route Tables"


class SecurityListsResource(_OptionalScopeKwargMixin, OciListResource):
    SCOPE_KWARG_NAME = "vcn_id"
    CLIENT_CLS = oci.core.VirtualNetworkClient
    SERVICE_NAME = "Virtual Network"
    TABLE_NAME = "virtual_network_security_lists"
    LIST_METHOD = "list_security_lists"
    GET_METHOD = "get_security_list"
    GET_ID_PARAM = "security_list_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "vcn_id"]
    SECTION_TITLE = "Security Lists"


class NetworkSecurityGroupsResource(_OptionalScopeKwargMixin, OciListResource):
    SCOPE_KWARG_NAME = "vcn_id"
    CLIENT_CLS = oci.core.VirtualNetworkClient
    SERVICE_NAME = "Virtual Network"
    TABLE_NAME = "virtual_network_network_security_groups"
    LIST_METHOD = "list_network_security_groups"
    GET_METHOD = "get_network_security_group"
    GET_ID_PARAM = "network_security_group_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "vcn_id"]
    SECTION_TITLE = "Network Security Groups (NSGs)"


class InternetGatewaysResource(_OptionalScopeKwargMixin, OciListResource):
    SCOPE_KWARG_NAME = "vcn_id"
    CLIENT_CLS = oci.core.VirtualNetworkClient
    SERVICE_NAME = "Virtual Network"
    TABLE_NAME = "virtual_network_internet_gateways"
    LIST_METHOD = "list_internet_gateways"
    GET_METHOD = "get_internet_gateway"
    # SDK signature is get_internet_gateway(self, ig_id, **kwargs) -- confirmed
    # via oci/core/virtual_network_client.py:get_internet_gateway.
    GET_ID_PARAM = "ig_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "vcn_id", "is_enabled"]
    SECTION_TITLE = "Internet Gateways"


class NatGatewaysResource(_OptionalScopeKwargMixin, OciListResource):
    SCOPE_KWARG_NAME = "vcn_id"
    CLIENT_CLS = oci.core.VirtualNetworkClient
    SERVICE_NAME = "Virtual Network"
    TABLE_NAME = "virtual_network_nat_gateways"
    LIST_METHOD = "list_nat_gateways"
    GET_METHOD = "get_nat_gateway"
    GET_ID_PARAM = "nat_gateway_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "vcn_id", "block_traffic"]
    SECTION_TITLE = "NAT Gateways"


class ServiceGatewaysResource(_OptionalScopeKwargMixin, OciListResource):
    SCOPE_KWARG_NAME = "vcn_id"
    CLIENT_CLS = oci.core.VirtualNetworkClient
    SERVICE_NAME = "Virtual Network"
    TABLE_NAME = "virtual_network_service_gateways"
    LIST_METHOD = "list_service_gateways"
    GET_METHOD = "get_service_gateway"
    GET_ID_PARAM = "service_gateway_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "vcn_id"]
    SECTION_TITLE = "Service Gateways"


class DrgsResource(OciListResource):
    CLIENT_CLS = oci.core.VirtualNetworkClient
    SERVICE_NAME = "Virtual Network"
    TABLE_NAME = "virtual_network_drgs"
...
class DrgAttachmentsResource(_OptionalScopeKwargMixin, OciListResource):
    SCOPE_KWARG_NAME = "drg_id"
    CLIENT_CLS = oci.core.VirtualNetworkClient
    SERVICE_NAME = "Virtual Network"
    TABLE_NAME = "virtual_network_drg_attachments"
    LIST_METHOD = "list_drg_attachments"
    GET_METHOD = "get_drg_attachment"
    GET_ID_PARAM = "drg_attachment_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "drg_id", "network_details"]
    SECTION_TITLE = "DRG Attachments"


class DhcpOptionsResource(_OptionalScopeKwargMixin, OciListResource):
    SCOPE_KWARG_NAME = "vcn_id"
    CLIENT_CLS = oci.core.VirtualNetworkClient
    SERVICE_NAME = "Virtual Network"
    TABLE_NAME = "virtual_network_dhcp_options"
    LIST_METHOD = "list_dhcp_options"
    GET_METHOD = "get_dhcp_options"
    GET_ID_PARAM = "dhcp_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "vcn_id"]
...
class InstanceConfigurationsResource(OciListResource):
    CLIENT_CLS = oci.core.ComputeManagementClient
    SERVICE_NAME = "Compute Management"
    TABLE_NAME = "compute_instance_configurations"
    LIST_METHOD = "list_instance_configurations"
    GET_METHOD = "get_instance_configuration"
    GET_ID_PARAM = "instance_configuration_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "time_created"]


class InstancePoolsResource(OciListResource):
    CLIENT_CLS = oci.core.ComputeManagementClient
    SERVICE_NAME = "Compute Management"
    TABLE_NAME = "compute_instance_pools"
    LIST_METHOD = "list_instance_pools"
    GET_METHOD = "get_instance_pool"
    GET_ID_PARAM = "instance_pool_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "size", "instance_configuration_id"]


class ClusterNetworksResource(OciListResource):
    CLIENT_CLS = oci.core.ComputeManagementClient
    SERVICE_NAME = "Compute Management"
    TABLE_NAME = "compute_cluster_networks"
    LIST_METHOD = "list_cluster_networks"
    GET_METHOD = "get_cluster_network"
    GET_ID_PARAM = "cluster_network_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "time_created"]


class ComputeClustersResource(OciListResource):
    # NOTE: list_compute_clusters/get_compute_cluster live on ComputeClient in
    # the pinned SDK (requirements.txt pins oci==2.181.1), not
    # ComputeManagementClient -- confirmed via
    # oci/core/compute_client.py:{list,get}_compute_cluster. The previous
    # CLIENT_CLS = ComputeManagementClient meant list_compute_clusters never
    # existed on the client, so the hasattr guard that used to live here
    # always failed and silently returned zero rows.
    CLIENT_CLS = oci.core.ComputeClient
    SERVICE_NAME = "Compute"
    TABLE_NAME = "compute_compute_clusters"
    LIST_METHOD = "list_compute_clusters"
    GET_METHOD = "get_compute_cluster"
    GET_ID_PARAM = "compute_cluster_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "time_created"]
...
class ComputeInstancesResource(OciListResource):
    """Compute instances.

    Also carries a handful of extra client methods with zero callers today
    (list_shapes/launch_instance/update_instance_metadata/instance_action/
    list_vnic_attachments) -- staged capability for a future compute exploit
    module, kept as-is.
    """
    ...
    GET_ID_PARAM = "instance_id"
    COLUMNS = ["id", "display_name", "lifecycle_state", "shape", "availability_domain"]

    # NOTE (behavior change / improvement): the pre-migration ``list_instances``
    # wrapper called ``self.client.list_instances(compartment_id=...)`` directly
    # -- a single page only, unlike every other list_* method in this codebase.
    # The inherited base ``_list_items`` paginates via
    # ``oci.pagination.list_call_get_all_results``, so large compartments now
    # get every page instead of silently truncating at page 1.
    ...
    def list_subscribed_regions(session, proxy: Optional[str] = None) -> List[str]:
        ...
        Delegates to the shared, session-cached region-subscription helper so the
        authoritative list is fetched once per session and reused by any region
        fan-out (see ``core.utils.regions``).
        """
        from ocinferno.core.utils import regions as _regions

        return _regions.subscribed_regions(session, proxy=proxy)


class ComputeImagesResource(OciListResource):
    CLIENT_CLS = oci.core.ComputeClient
    SERVICE_NAME = "Compute"
    ...
    COLUMNS = ["id", "display_name", "lifecycle_state", "operating_system", "operating_system_version"]

    # Same single-page -> paginated improvement as ComputeInstancesResource (the
    # original list_images also called the SDK directly, single page only). The
    # original's optional list filters (display_name/operating_system/shape/...)
    # are never actually passed by the enum wrapper -- base _list_items already
    # forwards **kwargs generically if a future caller ever needs them.

    def list(self, *, compartment_id: str, **kwargs) -> List[Dict[str, Any]]:
        rows = super().list(compartment_id=compartment_id, **kwargs)
        for row in rows:
            if isinstance(row, dict):
                # Normalize compartment_id into each row for DB joins (list_images
                # summaries don't otherwise carry it).
                row.setdefault("compartment_id", compartment_id)
        return rows


# =============================================================================
# Compute Instance Agent (run-command / execution / plugin machinery)
# =============================================================================
# Kept hand-rolled: 3 SDK clients, no single list/get pair, heavy per-row
# normalization -- not a fit for OciListResource's config-driven shape.
